...
typedef struct {
  ...
  size_t mid_point;  /**< Index of the peak's highest point (or middle of
                        plateau) */
 ...
 *
 * Prominence quantifies how much a peak stands out from the surrounding
 * baseline. This structure contains the prominence value and the indices of the
 * left and right base points used to calculate it.
 */
typedef struct {
  size_t left_base;  /**< Index of the left base point used for prominence
                        calculation */
  double prominence; /**< Calculated prominence value of the peak */
  size_t right_base; /**< Index of the right base point used for prominence
                        calculation */
...
typedef struct {
  ...
  double
      right_ip; /**< Interpolated position of the right width crossing point */
} whlr_peak_width_t;
 ...
 * This structure contains the threshold values for the left and right sides of
 * the peak.
 */
typedef struct {
  double left_threshold;  /**< Height difference between peak and left neighbor
                             valley */
  double right_threshold; /**< Height difference between peak and right neighbor
                             valley */
} lr_peak_threshold_t;
...
/**
 * @class RangeFloat
 * @brief Represents a range of floating-point values with lower and upper
 * bounds
 *
...
public:
   ...
   * @details Sets the lower bound to the specified value and upper bound to
   * maximum double
   */
   ...
   *
   * This operator allows easy string conversion for debugging or display
   * purposes
   */
...
/**
 * @class RangeInt
 * @brief Represents a range of integer values (size_t) with lower and upper
 * bounds
 *
 * Similar to RangeFloat, this class provides flexible numeric ranges for
 * integer values, supporting various initialization methods and string
 * conversion.
 */
class RangeInt {
public:
 ...
 * @brief Configures comprehensive criteria for peak detection in signal
 * processing
 *
...
public:
   ...
   * @param plateau_size Range of acceptable plateau sizes (default: entire
   * range)
   *
   * Allows flexible initialization of peak detection parameters with sensible
   * defaults
   */
  PeakConditions(const RangeFloat &height = RangeFloat(),
                 ...
                 const RangeFloat &width = RangeFloat(), size_t wlen = 0,
   ...
   */
  template <class... Args> void set_height(Args &&...args) {
    this->height = RangeFloat(std::forward<Args>(args)...);
  }
   ...
   */
  template <class... Args> void set_threshold(Args &&...args) {
    this->threshold = RangeFloat(std::forward<Args>(args)...);
  }
   ...
   */
  template <class... Args> void set_prominence(Args &&...args) {
    this->prominence = RangeFloat(std::forward<Args>(args)...);
  }
   ...
   */
  template <class... Args> void set_width(Args &&...args) {
    this->width = RangeFloat(std::forward<Args>(args)...);
  }
   ...
   */
  template <class... Args> void set_plateau_size(Args &&...args) {
    this->plateau_size = RangeInt(std::forward<Args>(args)...);
  }
};
 ...
 *
 * @return Vector of peak_result_t structures containing detailed peak
 * information
 *
 * @details
 * The function performs sophisticated peak detection with the following key
 * features:
 ...
 */
std::vector<int>
find_peaks(const std::vector<double> &x, ///< Input signal vector
           PeakConditions conditions =
               PeakConditions() ///< Peak detection conditions (optional)
);
